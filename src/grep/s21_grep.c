...
void return_text(char *string, struct string_node *list[],
...
void get_paths(const char *argv[], struct string_node *path_names[], int * flag_num) {
    char *path = NULL;
...
void s21_grep(int argc, char *argv[]) {
    ...
    if (pattern == NULL && !(flag & FLAG_REGEXP))
        pattern = argv[optind++];
    ...
    if (path_names == NULL) {
        ...
        while ((path_name = pop_node(&path_names)) != NULL) {
            if (what_is(path_name) == FILE_EXISTS) {
                ...
                stream_grep(stream,
                            (num_nodes > 1) ? PRINT_FILE : DO_NOT_PRINT_FILE,
                            path_name);
                fclose(stream);
            } else {
                flag &= ~FLAG_LAST_CHAR;
